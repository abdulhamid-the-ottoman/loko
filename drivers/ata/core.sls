...
(define-inlined ata-status-SERV  #b00010000) ;service (obsolete)
(define-inlined ata-status-DRQ   #b00001000) ;data request
(define-inlined ata-status-CORR  #b00000100) ;corrected data (obsolete)
(define-inlined ata-status-IDX   #b00000010) ;index bit (obsolete)
(define-inlined ata-status-ERR   #b00000001) ;error

(define-inlined ata-control-HOB  #b10000000) ;high order byte
(define-inlined ata-control-SRST #b00000100) ;software reset
(define-inlined ata-control-nIEN #b00000010) ;interrupt enable

;; Newer definitions
(define-inlined ata-status-STRE   #b00100000) ;stream error
(define-inlined ata-status-DFE    #b00010000) ;deferred write error
(define-inlined ata-status-AE     #b00000100) ;alignment error
(define-inlined ata-status-SENSE  #b00000010) ;sense data available
(define-inlined ata-error-TIMEOUT #b00000001) ;command completion timeout
...
(define ata-cmd-read-fpdma-queued          #x60) ;NCQ
(define ata-cmd-write-fpdma-queued         #x61) ;NCQ
...
(define (ata-WRITE-SECTORS dev lba sectors data)
  (assert (fx<=? 1 sectors 256))
  (assert (eqv? lba (fxbit-field lba 0 28)))
  (let ((sector-count (if (eqv? sectors 256) 0 sectors)))
    (make-cmd-pio-data-out data
                           (make-inputs 0 sector-count lba
                                        (fxarithmetic-shift-left ata-device-LBA 8)
                                        ata-cmd-write-sectors))))
...
;;; Extra commands for the PACKET feature set
