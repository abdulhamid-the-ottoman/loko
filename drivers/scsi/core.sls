...
(define (probe·scsi request-ch)
  (match (!? request-ch (scsi-INQUIRY 5))
    [('ok _ resp)
     (scsi-test-unit-ready request-ch)
     (send-log DEBUG (call-with-string-output-port
                       (lambda (p)
                         (display "SCSI SENSE in probe: " p)
                         (write (scsi-get-sense request-ch) p))))
...
;; Ask the logical unit if it's ready. Returns #f if there is sense
;; data to read with scsi-get-sense.
(define (scsi-test-unit-ready request-ch)
  (match (!? request-ch (scsi-TEST-UNIT-READY))
    [('ok resp) #t]
    [('error . x) #f]))
...
(define (scsi-TEST-UNIT-READY)
  (let ((control 0))
    (list 'non-data (pack "!uC4xC" SCSI-TEST-UNIT-READY control))))
