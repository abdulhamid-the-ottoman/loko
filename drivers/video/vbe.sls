...
(define (probe·pci·vbe? dev)
  (and (eqv? (pcidev-base-class dev) #x03)
       ...
       (let lp ((bridge (pcidev-bridge dev)))
         (cond ((not bridge)
                #t)
               ((eqv? 0 (fxand (pci-get-u16 bridge PCI-CFG-01-BRIDGE-CONTROL)
                               PCI-BCTRL-VGA-ENABLE))
                ;; The bridge does not forward VGA accesses, so this
                ;; can't be the active VGA device.
                #f)
               (else
...
(define (video-bios-init dev img)
  (let ((M (make-machine)))
    ...
    (cond ((pcirom? img)
           ;; The PCI device has an expansion ROM
           (copy-to-memory M #xC0000 (pcirom-image img)))
          ((eqv? (get-mem-u16 #xC0000) #xAA55)
           ;; Copy the Video BIOS (plus the rest of the ROMs) from the
           ;; system.
           (do ((RAM (machine-RAM M))
                (addr #xC0000 (fx+ addr 4)))
               ((fx=? addr #x100000))
             (memory-u32-set! RAM addr (get-mem-u32 addr))))
          (else
           (error 'video-bios-init "No Video BIOS found" dev)))
