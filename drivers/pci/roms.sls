      ...
      (with-BARs-saved dev
        (lambda ()
          (let ((size (pcidev-ROM-size dev)))
            (if (not size)
                #vu8()
                (cond ((pcidev-map-ROM dev) =>
                       (lambda (addr)
                         (with-pcibridge-memory (pcidev-bridge dev) addr size
                           (lambda ()
                             ;; Copy the ROM and get out asap
...
(define-record-type pcirom
  (nongenerative pcirom-9cdd5610-48e1-4329-a792-50fd2ff75cba)
  (sealed #f)
  (fields image
