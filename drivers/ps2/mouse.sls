...
(define (parse-mouse-flags flags x y)
  ...
  (let ((y-overflow (fxand flags #b10000000))
    ...
    ;; XXX: the y axis is inverted, proving that reverse mouse was
    ;; always the correct option in games
    (values (decode-signed-9-bit (fxarithmetic-shift-right x-sign 4) x)
            (fx- (decode-signed-9-bit (fxarithmetic-shift-right y-sign 5) y)))))
