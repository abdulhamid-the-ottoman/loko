...
(library (loko compiler compat)
  ...
  (import
    ...
    (rnrs files)
    (rnrs unicode)
    (rename (only (ikarus) gensym? gensym->unique-string gensym-prefix call/cc)
            (call/cc call/1cc)
            (gensym->unique-string ik:gensym->unique-string))
    (loko config)
...
(define gensym->unique-string ik:gensym->unique-string)
