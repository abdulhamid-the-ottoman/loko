...
(define (pass-loops x)
  ...
  (define (misused? x tail? name formal*)
    ...
    (cond ((bind? x)
          ...
          ((mv-call? x)
           (or (misused? (mv-call-producer x) #f name formal*)
               (misused? (mv-call-consumer x) #f name formal*)))
          ((mv-let? x)
           (or (misused? (mv-let-expr x) #f name formal*)
               (misused? (mv-let-body x) tail? name formal*)))
          ((mv-values? x)
           (exists (lambda (x) (misused? x #f name formal*))
                   (mv-values-expr* x)))
          (else
  ...
  (define (captured? x in-lambda? name)
    ...
    (cond ((bind? x)
          ...
          ((mv-call? x)
           (or (captured? (mv-call-producer x) in-lambda? name)
               (captured? (mv-call-consumer x) in-lambda? name)))
          ((mv-let? x)
           (or (captured? (mv-let-expr x) in-lambda? name)
               (captured? (mv-let-body x) in-lambda? name)))
          ((mv-values? x)
           (exists (lambda (x) (captured? x in-lambda? name))
                   (mv-values-expr* x)))
          (else
             ...
             (cond ((and (ref? op) (eq? (ref-name op) name))
                   ...
                   (else
                    (make-funcall (rewrite op)
                                  ...
                                  (funcall-source x))))))
          ((mv-call? x)
           (make-mv-call (rewrite (mv-call-producer x))
                         (rewrite (mv-call-consumer x))
                         (mv-call-source x)))
          ((mv-let? x)
           (make-mv-let (rewrite (mv-let-expr x))
                        (mv-let-lhs* x)
                        (rewrite (mv-let-body x))
                        (mv-let-source x)))
          ((mv-values? x)
           (make-mv-values (map rewrite (mv-values-expr* x))
                           (mv-values-source x)))
          ((const? x) x)
          ((ref? x) x)
          ((primref? x) x)
