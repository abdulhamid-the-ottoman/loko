...
;;; Plot the performance of bytevector-sint-set! with little endianness
...
(do ((i -128 (+ i 1))
     ...
     (counts '()
             (cons (cons i
                         (time-it* (string-append "bytevector-sint-set! little "
                   ...
                   counts)))
    ((= i 128)
     (call-with-output-file "tests/perf/bytevector-sint-le-set_.csv"
       (lambda (p)
         (display "# bits, cycles\n" p)
